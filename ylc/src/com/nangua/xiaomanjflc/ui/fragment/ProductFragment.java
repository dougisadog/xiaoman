...
//	private LayoutInflater inflater;
	...
	private SparseArrayCompat<RelativeLayout> topButtons = new SparseArrayCompat<RelativeLayout>();
...
//									.getTotalInvestment());
				...
				float rateSubsidy = 0;
				try {
					rateSubsidy = Float.parseFloat(item.getRateSubsidy());
				} catch (NumberFormatException e) {
					// TODO Auto-generated catch block
					e.printStackTrace();
				}
				if ("1".equals(item.getRateFlg()) && rateSubsidy > 0) {
					confine.setImageResource(R.drawable.confine9);
				}
				else {
...
//				new_hand = ret.getInt("new_hand");
				...
				if (page == 1) {
					products = new ProductList(arr)
							.getProducts();
				} else {
					products = new ProductList(products, arr).getProducts();
				}
				adapter.setList(products);
				mEmpty.setVisibility(products.size() > 0 ? View.INVISIBLE : View.VISIBLE);
				String content = "默认";
