...
import com.nangua.xiaomanjflc.ui.RedPlusActivity;
...
public class AccountFragment extends HomeFragment {
	...
	private LinearLayout btnSwitchTotal;
	...
	private int currentEye = EYEON;
	
	private static final int EYEON = 1;
	private static final int EYEOFF = 2;
	...
	public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container,
		...
		imgEye = (ImageView) v.findViewById(R.id.imgEye);
		
		TextView titleTotal = (TextView) v.findViewById(R.id.titleTotal);
		initEyeImage(titleTotal);
		
		btnSwitchTotal = (LinearLayout) v.findViewById(R.id.btnSwitchTotal);
		btnSwitchTotal.setOnClickListener(listener);
...
//				getActivity().startActivityForResult(new Intent(getActivity(), RedActivity.class), MainActivity.REQUEST_CODE);
				getActivity().startActivityForResult(new Intent(getActivity(), RedPlusActivity.class), MainActivity.REQUEST_CODE);
				break;
			case R.id.invite:
	...
	private void switchTotal() {
		if (null == mTotal.getTag()) return;
		String total = mTotal.getTag().toString();
		if (currentEye == EYEON) {
			imgEye.setImageResource(R.drawable.icon_eye_hide);
			
			mTotal.setText(FormatUtils.numSecretHide(total));
			currentEye = EYEOFF;
			return;
		}
		if (currentEye == EYEOFF) {
			imgEye.setImageResource(R.drawable.icon_eye_show);
	...
	private void refreshGainData(GainData gainData) {
		...
		String total = FormatUtils.fmtMicrometer(FormatUtils.priceFormat(available + investAmount + frozeAmount));
		mTotal.setText(total);
		mTotal.setTag(total);
...
//			taskGotMessage();
	...
	/**
	 * 
	 * @param forceUpdate  true 强制更新  false 需要检测
	 */
